...
@pytest.fixture
def vault():
    return Vault(os.path.join(HERE, "data/vault"))


@pytest.fixture
def saved_models():
    config = MagicMock()
    config.models_dir.return_value = os.path.join(HERE, "data/models")
...
def test_images(vault: Vault):
    ...
    for full_path in all_files:
        rel_path = os.path.relpath(full_path, vault.path)
...
def test_deep_features_extractor(saved_models, deep_extractor, test_images):
    ...
    # res.to_csv(os.path.join(TEST_DIR, "ref_deep_features.csv"))
    # Compare with expected
    expected = pd.read_csv(os.path.join(HERE, "data/ref_deep_features.csv"), index_col="id")
