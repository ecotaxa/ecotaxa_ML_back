...
def common_mocks(monkeypatch):
    ...
    monkeypatch.setattr("ml_jobs_runner.Config", MagicMock(return_value=mock_config))

    mock_connection = MagicMock()
    monkeypatch.setattr("ml_jobs_runner.Connection", mock_connection)

    mock_gpu_predict = MagicMock()
    mock_gpu_predict.JOB_TYPE = "Prediction"
    monkeypatch.setattr("ml_jobs_runner.GPUPredictForProject", mock_gpu_predict)

    mock_scheduler_module = MagicMock()
    monkeypatch.setattr("ml_jobs_runner.JobScheduler", mock_scheduler_module)

    mock_scheduler_cls = MagicMock(spec=JobScheduler)
    monkeypatch.setattr("ml_jobs_runner.JobSchedulerClass", mock_scheduler_cls)
    ...
    monkeypatch.setattr("ml_jobs_runner.time.sleep", mock_sleep)
...
def test_main_one_shot_no_job(monkeypatch, common_mocks):
    ...
    # Simulate no job found
    mock_scheduler_instance._run_one.side_effect = None
